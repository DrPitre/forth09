...
init_dict()
{
     ...
     int equal(), less_than(), greater_than(), zero_equal(), zero_less() ;
     int zero_greater(), not(), and(), or(), quest_dup(), abort_quote() ;
     int quest_stack(), iffunct(), elsefunct(), thenfunct() ;
     int inc(), dec(), inc2(), dec2(), left_shift(), right_shift(), absfunct() ;
     int negate(), forth_min(), forth_max(), to_return(), from_return(), copy1_return() ;
     ...
     temp->word = "MIN" ;
     temp->code_adr.adr = forth_min ;
     temp = temp->dict_next ;

     temp->word = "MAX" ;
     temp->code_adr.adr = forth_max ;
     temp = temp->dict_next ;
